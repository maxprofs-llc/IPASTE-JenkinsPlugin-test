...
public class HelloWorldBuilder extends Builder implements SimpleBuildStep {
    ...
    public HelloWorldBuilder(String command) {
    ...
    public void perform(Run<?,?> build, FilePath workspace, Launcher launcher, TaskListener listener) throws IOException, InterruptedException {
        ...
        // This also shows how you can consult the global configuration of the builder
            ...
            FilePath file = new FilePath(workspace,"Test.txt");
            file.write("Test","UTF-8");

            BatchFile batchFile = new BatchFile("echo Running script\r\n" +
                    command);
            FilePath tmpFile = batchFile.createScriptFile(workspace);
            int exit = launcher.launch().cmds(batchFile.buildCommandLine(tmpFile)).stdout(listener).pwd(workspace).join();
            listener.getLogger().println("Ran command. Got exit code " + exit);
    }
